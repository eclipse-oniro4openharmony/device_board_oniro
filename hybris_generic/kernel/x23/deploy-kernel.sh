...
# This script automates the deployment of kernel modules and boot images to the Volla X23 device.
# Sudo password on the device is expected to be '1234'.

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OHOS_SOURCE_ROOT="$(cd "$HERE/../../../../../.." && pwd)"
KERNEL_OUT="$OHOS_SOURCE_ROOT/kernel/linux/volla-vidofnir/out"
MODULES_SRC="$OHOS_SOURCE_ROOT/kernel/linux/volla-vidofnir/build-dir/tmp/system/lib/modules"
...
if ! adb devices | grep -q "[0-9a-fA-F]\+[[:space:]]\+device$"; then
    # Some devices might not have a serial number in 'adb devices' output as seen before
