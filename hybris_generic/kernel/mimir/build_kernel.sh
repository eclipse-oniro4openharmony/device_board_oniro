...
# Copyright (c) 2025-2026 Eclipse Oniro for OpenHarmony contributors.
...
#
# All kernel modifications live under kernel/mimir/ — this script is their
# single owner (system_patch/ deliberately does not touch the kernel; the
# kernel-port repo and the MT8781 source are not OHOS-checkout repos).
# There are two distinct target trees, hence two patch sets — see
# patches/README.md:
#   patches/port-repo/     -> the volla-mimir clone ($KERNEL_TREE)
#   patches/kernel-source/ -> the downloaded MT8781 source (temp copy)
