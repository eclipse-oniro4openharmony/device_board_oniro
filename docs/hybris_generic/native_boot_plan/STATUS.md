...
| N4 | `phase_n4_androidd.md` | 🟡 Original plan was Android-as-guest namespace.  Not used in the current native build (we don't run Android HALs natively yet — graphics, audio, sensors still pending).  Removed from the build (was unused). |
| N5 | `phase_n5_android_image.md` | 🟡 Same as N4 — pending the Phase N8 graphics work. |
| N6 | `phase_n6_binder.md` | ✅ Vendor cfg ships separate `binder` device-manager nodes.  Unused on the current native build (no Android guest), but kept for Phase N8. |
...
| N8 | `phase_n8_graphics_native.md` | 🔄 Open.  Native composer_host has no `/android/vendor/lib64/hw/` to bridge to — needs different display HAL approach. |
...
- **Phase N8 (display).**  Composer_host / render_service expect HALs at
  `/android/vendor/lib64/hw/` from the LXC build's `/android` bind
  mount.  Native has none of that.  Needs either native HAL impls
  (large) or an Android-guest namespace inside native OHOS (the
  original Phase N4 idea, rebooted).
- **Phase N9 (peripherals).**  WiFi/audio rely on the same Android-side
  HALs; same blocker as N8.
- **The original phase docs** (`phase_n*.md`) capture the design
  thinking we went through and are kept as historical context.  Where
  they describe an approach that was replaced (notably N1/N4/N5),
  treat them as design history, not current state.
