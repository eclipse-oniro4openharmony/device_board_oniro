...
| **N12.6.1a** | Server-side binder in our process so `ICameraDevice::open(sp<ICameraDeviceCallback>)` accepts a working callback ref.  Worker thread `BC_ENTER_LOOPER`s + dispatches `BR_TRANSACTION` (in `hw_binder_server.{h,cpp}`); client also handles inline `BR_TRANSACTION` from nested-call (kernel routes Halium's callback to the same thread that's waiting on open()'s BR_REPLY).  `HwCameraDeviceCallback` stubs IBase + camera methods. | ✅ DONE (2026-05-21) |
| **N12.6.1b** | `HwCameraDevice::Open` (V3.2 TX 4) bridges to Halium → ICameraDeviceSession handle returned; `HwCameraDeviceSession::ConstructDefaultRequestSettings` (V3.2 TX 1) returns 2592 bytes of PREVIEW template metadata, proving session is responsive. | ✅ DONE (2026-05-21) |
| N12.6.1c | `configureStreams_3_4` (V3.4 TX 9) nested struct serialiser: `V3_4::StreamConfiguration { V3_2::StreamConfiguration { hidl_vec<V3_2::Stream> streams; StreamConfigurationMode mode; }; hidl_vec<uint8> sessionParams; uint32 streamConfigCounter; }` plus `HalStreamConfiguration` reply parser.  ~500 lines of HIDL nested-struct serialisation work. | ⏳ |
| N12.7a | FMQ (Fast Message Queue) shared-memory ring buffers via `getCaptureRequestMetadataQueue` (V3.2 TX 4) + `getCaptureResultMetadataQueue` (V3.2 TX 5).  `MQDescriptor<uint8, kSync>` over hidl_handle (ashmem fd).  Used by processCaptureRequest_3_4 to avoid copying camera_metadata. | ⏳ |
| N12.7b | `processCaptureRequest_3_4` (V3.4 TX 10): `hidl_vec<CaptureRequest>` + `hidl_vec<BufferCache>`.  Each CaptureRequest carries frame_number, fmqSettingsSize, settings, hidl_vec<StreamBuffer> output_buffers — and StreamBuffer holds a native_handle_t * (Halium gralloc handle). | ⏳ |
| N12.7c | `processCaptureResult` callback dispatch (already arrives via our HwBinderServer): parse CaptureResult, map Halium native_handle_t → OHOS BufferHandle via allocator_host's libhybris-bridged gralloc, push to the attached `BufferProducerSequenceable` — frames appear on the OHOS Camera HAP preview surface. | ⏳ |
