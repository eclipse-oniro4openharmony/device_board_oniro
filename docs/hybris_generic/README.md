...
| 8 | [System Stability](phase8_system_stability.md) | 🔄 In Progress | Unlock, tap, and display composition bugs fixed (8.1–8.9, 8.12–8.18); power button shutdown fixed (systemd-logind `HandlePowerKey=ignore` + `DoSuspend()` container bypass); Bug 8.16: all layers start as CLIENT composition; Bug 8.17: cross-thread eglSwapBuffers/eglDestroySurface race fixed (rwlock in libhybris EGL) — but the same `NULL+0x1d8` Mali crash still reproduces on dropdown open/close on the SAME `RSRenderThread` (2026-04-17 regression; root cause not destroy itself, see 8.17 doc for what's been ruled out); Bug 8.18: webview/nweb render fixed via `chmod 0644` on sandbox configs (nwebspawn uid 3081 couldn't read root-only files); Bug 8.11 (`SetLayerAlpha` UAF) not yet fixed |
...
| 13 | [Audio Support](phase13_audio_support.md) | ✅ 13B Complete on tablet (2026-04-16) + X23 (2026-04-17) | Native ALSA via alsa-lib: `audio_host` + `libaudio_render_adapter.z.so` + `libasound` + `/dev/snd/pcmC0D0p`, MT6789 DAPM route bring-up in `vendor_render.c::RenderInitImpl`, blocking PCM, period-time-before-buffer-time, start_threshold=period_size, `snd_ctl_close` leak-fix — and the final ingredient for the tablet: **`snd_pcm_close`+`snd_pcm_open` on every Start in `RenderStartImpl`**. X23 port (2026-04-17): common.h numids set to 0 so `SetElementInfo` falls back to name lookup (X23 kernel inserts extra codec controls, shifting numids by +2 from the tablet); added X23 speaker path `DL1 → I2S3 → AW883xx smart PA` (`I2S3_CH{1,2} DL1_CH{1,2}`, `I2S3_Out_Mux=Normal`, `aw_dev_0_switch=Enable`) — tablet `Ext_Speaker_Amp` path left in place (harmless on X23 where Ext_Speaker_Amp actually feeds the headphone jack). 13A (libhybris→MTK HAL) stays stashed as fallback. |
...
**8.17:** `com.ohos.systemui` SIGSEGV in Mali `eglSwapBuffers` when dismissing the dropdown panel. **Partial fix (2026-04-09):** added `pthread_rwlock_t` in `egl.c` so `eglSwapBuffers` takes a read lock and `eglDestroySurface` takes a write lock (plus a `std::mutex` around `helper.cpp`'s `_surface_window_map`). This closes the cross-thread destroy-vs-swap race described in the original crash-log timeline and stays deployed. **Reopened (2026-04-17):** the identical `NULL+0x1d8` crash signature still reproduces on dropdown open/close. Follow-up investigation confirmed both the destroying path (`RSSurfaceOhosGl::ClearBuffer`, posted to `RSRenderThread` via `RSUIDirector::GoBackground`) and the swapping path (`RSSurfaceOhosGl::FlushFrame`, VSync-driven on `RSRenderThread`) run on the *same* EventHandler, so the rwlock is a no-op for this workload. Ruled out: pending-GPU-work (tested `eglWaitGL()` inside the write-locked destroy — no improvement) and Mali's `_eglDestroySurface` call itself (tested skip-destroy leaking the EGLSurface metadata — the same crash returned after ~8 min of organic use, MD5-verified the skip was active). Remaining candidates: `DestoryNativeWindow(mWindow)` before the teardown, `producer_->GoBackground()` after, surfaceless `eglMakeCurrent` transition, or GL-side resource deletes in ArkUI teardown. See `phase8_system_stability.md` §8.17 "Regression — Re-opened 2026-04-17" for the detailed rulings and the concrete next-session instrumentation plan.
