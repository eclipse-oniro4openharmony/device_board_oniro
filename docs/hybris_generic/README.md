...
| N12 | [phase_n12_camera.md](phase_n12_camera.md) + **[phase_n12_camera_droidmedia.md](phase_n12_camera_droidmedia.md)** (architecture pivot, 2026-05-21) | 🔄 **N12.D droidmedia pivot landed (2026-05-21).**  New `Droid::Loader` singleton `hybris_dlopen`s `/android/system/lib64/libdroidmedia.so` + binds `/dev/binderfs/android-binder` over `/dev/binder` in a private mount NS (required because AOSP libbinder's `ProcessState::self()` inside libdroidmedia hard-codes `open("/dev/binder")` and on the OHOS side that's a different driver).  3 cameras enumerate end-to-end through the loader (`droidmedia_smoke` + `droidmedia_loader_smoke` both green; rear primary S5KGM1ST, front OV16A1Q, rear wide GC08A3WIDE — `facing`/`orientation` match the hand-rolled X23 ability table).  New `HybrisCameraHostVdiImpl` / `HybrisCameraDeviceVdiImpl` / `HybrisStreamOperatorVdiImpl` replace the hand-rolled HIDL camera device path (`hw_camera_provider`/`hw_camera_device`/`hw_camera_device_session`/`hw_camera_device_callback` files DELETED; generic `hw_binder_*` + `hw_service_manager` kept for future sensors/BT HIDL).  Stream operator implements `CommitStreams` (preview-size `setParameters`), preview frame copy (NV21/NV12 → BufferProducer surface), `take_picture` (JPEG via `compressed_image_cb`).  Production wiring: `CAP_SYS_ADMIN` added to the `camera_host` host block in `device_info.hcs`; `HYBRIS_LD_LIBRARY_PATH` (including `/android/system/apex/com.android.i18n/lib64` where libandroidicu lives) seeded via `setenv` in `Droid::Loader::Init`.  Earlier N12.x context (kernel module bundling, `/mnt/vendor/nvcfg` mount via `mount_vendor_nv` in `androidd.c`, the 3-camera `dumpsys media.camera`) all preserved in the per-phase doc.  **Pending end-to-end verification:** Camera HAP frames-on-screen — gated on flashing the `super.img` rebuild that carries the new caps + VDI.  Torch (N12.D.9) deferred — `droid_media_camera_set_torch_mode` symbol absent on this libdroidmedia revision. |
