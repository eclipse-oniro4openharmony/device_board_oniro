...
| N12 | [phase_n12_camera.md](phase_n12_camera.md) | 🔄 **In progress (2026-05-20).**  N12.1 ✅ + N12.2 ✅ + N12.2.5 ✅ + N12.4 ✅ landed.  18 MT6789 ISP6s camera kernel modules bundled into `vendor_boot` + `insmod`'d at pre-init from `init.x23.cfg`; 6 ISP sub-blocks deferred (watchdog reboot — see [modulemap](phase_n12_camera_modulemap.md)).  VDI scaffolding (`libcamera_host_vdi_impl_1.0.z.so`) under `device/soc/oniro/hybris_generic/hardware/camera/` loads + registers (hilog `[I] CAMERA_VDI VDI registered, cameraIds=lcam001`).  **N12.2.5 RESOLVED:** Halium camera HAL was returning 0 devices not because of sensor binding (i2c sensors DO bind and the kernel reads back correct sensor IDs — S5KGM1ST/OV16A1Q/GC08A3WIDE) but because `/mnt/vendor/nvcfg` (3A/sensor NV calibration) wasn't mounted under native boot.  `mount_vendor_nv()` in `androidd.c` setns'es into Halium's post-pivot mount NS and mounts it — `dumpsys media.camera` now reports 3 devices with full Camera3 characteristics, `ICameraProvider@2.6::getCameraIdList` returns the 3 cameras.  **Open:** N12.5+ libhybris ICameraProvider bridge to expose them through our VDI. |
